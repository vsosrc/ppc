...
class HdfsServiceCheck(Script):
  def service_check(self, env):
    ...
    sys.path.append(params.hadoop_bin_dir)
    ...
    test_dir_exists = format("/opt/vse/hadoop/bin/hadoop --config {hadoop_conf_dir} fs -test -e {dir}")
    ...
    create_file_cmd = format(
      "{cleanup_cmd}; /opt/vse/hadoop/bin/hadoop --config {hadoop_conf_dir} fs -put /etc/passwd {tmp_file}")
